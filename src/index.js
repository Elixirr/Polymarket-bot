...
// Conditional Binance imports
let fetchKlines, fetchLastPrice, startBinanceTradeStream;
if (!CONFIG.disableBinance) {
  const binanceModule = await import("./data/binance.js");
  fetchKlines = binanceModule.fetchKlines;
  fetchLastPrice = binanceModule.fetchLastPrice;
  const binanceWsModule = await import("./data/binanceWs.js");
  startBinanceTradeStream = binanceWsModule.startBinanceTradeStream;
}
...
async function main() {
  const binanceStream = !CONFIG.disableBinance ? startBinanceTradeStream({ symbol: CONFIG.symbol }) : null;
  ...
  while (true) {
    ...
    const wsTick = binanceStream ? binanceStream.getLast() : null;
    ...
    try {
      ...
      let klines1m = [], klines5m = [], lastPrice = null;
      if (!CONFIG.disableBinance) {
      ...
      const [chainlink, poly] = await Promise.all([
        chainlinkPromise,
      ...
      const rsiMa = hasCandleData ? sma(rsiSeries, CONFIG.rsiMaPeriod) : null;
      const rsiSlope = hasCandleData ? slopeLast(rsiSeries, 3) : null;

      const macd = hasCandleData ? computeMacd(closes, CONFIG.macdFast, CONFIG.macdSlow, CONFIG.macdSignal) : null;

      const ha = hasCandleData ? computeHeikenAshi(candles) : [];
      const consec = hasCandleData ? countConsecutive(ha) : [0, 0];

      const vwapCrossCount = hasCandleData ? countVwapCrosses(closes, vwapSeries, 20) : 0;
      const volumeRecent = hasCandleData ? candles.slice(-20).reduce((a, c) => a + c.volume, 0) : 0;
      const volumeAvg = hasCandleData ? (candles.slice(-120).reduce((a, c) => a + c.volume, 0) / 6) : 0;

      const failedVwapReclaim = vwapNow !== null && vwapSeries.length >= 3 && hasCandleData
      ...
      const macdLabel = (!macd || macd.hist === undefined)
        ? "-"
        : macd.hist < 0
      ...
      // Log signals to file for paper trading
      if (rec.action === "ENTER") {
        try {
          const signalData = {
            timestamp: Date.now(),
            ...
            market_title: poly.market?.question,
            prices: { up: prices.up, down: prices.down }
          };
          fs.appendFileSync("./signals.log", JSON.stringify(signalData) + "\n");
        } catch (e) {
          // ignore
        }
      }
      ...
      if (!CONFIG.disableBinance) {
        ...
        binanceSpotKvLine = kv("BTC (Binance):", binanceSpotValue);
      }
      ...
      const lines = [
        ...
        sepLine(),
        "",
        ...(binanceSpotKvLine ? [binanceSpotKvLine, ""] : []),
        sepLine(),
        "",
